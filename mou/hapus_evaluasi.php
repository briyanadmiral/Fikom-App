...
if ($id_eval > 0 && $id_pelaksanaan > 0) {
    ...
    $query = "UPDATE evaluasi_eksternal SET deleted_at = NOW() WHERE id_eval_eksternal = $id_eval";
    if (mysqli_query($conn, $query)) {
        header("Location: evaluasi2_pelaksanaan.php?id=$id_pelaksanaan&deleted_eval=1");
    } else {
        header("Location: evaluasi2_pelaksanaan.php?id=$id_pelaksanaan&error_eval=" . urlencode(mysqli_error($conn)));
    }
} else {
