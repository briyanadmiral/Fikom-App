...
@php
    ...
    if ($peranId === 1) {
        ...
        $baseQuery
            ->where('status_surat', 'disetujui') // Hanya lihat yang sudah final
...
@push('styles')
    <style>
        ...
        .page-header .icon {
            ...
            border-radius: 50%;
            box-shadow: 0 1px 10px #6610f24d;
            font-size: 2rem;
        }
        ...
        .online-user-avatar .avatar-icon {
            font-size: 1.3rem;
            color: #fff;
        }


        /* ✅ GRID UTAMA DASHBOARD: kiri & kanan rapi */
        .dashboard-main-grid {
            display: grid;
...
@section('content_header')
    <div class="page-header mt-2 mb-3">
        ...
        {{-- ✅ TAMBAH: Tombol Kembali ke Dashboard Menu --}}
        @if (session('entered_from_dashboard'))
            <form action="{{ route('external.exit') }}" method="POST" class="d-inline">
                @csrf
                <button type="submit" class="exit-dashboard-btn">
                    <i class="fas fa-sign-out-alt"></i> Kembali ke Dashboard Menu
                </button>
            </form>
        @endif
    </div>
...
@section('content')
    <div class="container-fluid pb-4">
        ...
        <div class="welcome-banner mb-4 shadow-sm">
            ...
            <img src="{{ asset('assets/img/Siega.png') }}" alt="Illustration" class="d-none d-sm-block">
        </div>
        ...
        <div class="row mb-4">
            ...
            'draft' => ['color' => 'secondary', 'icon' => 'fa-file-alt'],
                ...
                <div class="col-lg-3 col-6">
                    <div class="stat-card bg-{{ $info['color'] }} mb-3 mb-lg-0">
                        <div class="card-body">
        ...
        {{-- GRID UTAMA: kiri = aktivitas, kanan = akses cepat & online --}}
        <div class="dashboard-main-grid">
            {{-- KOLOM KIRI: Aktivitas Surat Terbaru --}}
            <div>
                <div class="card dashboard-card mb-4">
                    <div class="card-header">
                        <i class="fas fa-history mr-2"></i>Aktivitas Surat Terbaru Anda
                    </div>
                        ...
                        <ul class="list-group list-group-flush">
                            @forelse($recentSurat as $surat)
                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                    <div>
                                        <a href="{{ route('surat_tugas.show', $surat->id) }}"
                                            class="font-weight-bold text-dark">
                                            {{ $surat->nomor }}
                                        </a>
                                        <small class="d-block text-muted">
                                            {{ \Illuminate\Support\Str::limit($surat->nama_umum, 60) }}
                                            oleh {{ optional($surat->pembuat)->nama_lengkap ?? 'N/A' }}
                                        </small>
                                    </div>
                                    <span
                                        class="badge badge-pill badge-{{ $surat->status_surat == 'disetujui' ? 'success' : ($surat->status_surat == 'pending' ? 'warning' : 'secondary') }}">
                                        {{ $surat->status_surat }}
                                    </span>
                                </li>
                            @empty
                                <li class="list-group-item text-center text-muted py-4">
                                    Tidak ada aktivitas surat yang relevan untuk Anda.
                                </li>
                            @endforelse
                        </ul>
                    </div>
            ...
            {{-- KOLOM KANAN: Akses Cepat + Pengguna Online --}}
            <div>
                {{-- Akses Cepat --}}
                <div class="card dashboard-card mb-4">
                    <div class="card-header">
                    ...
                    <div class="card-body">
                        @if ($peranId == 1)
                            <div class="quick-action-item">
                                <a href="{{ route('surat_tugas.create') }}">
                                    <span class="quick-action-icon bg-primary">
                            ...
                            <div class="quick-action-item">
                                <a href="{{ route('surat_keputusan.create') }}">
                                    <span class="quick-action-icon" style="background-color: #3498db;">
                                        <i class="fas fa-plus"></i>
                                    </span>
                        ...
                        @if ($user->canApproveSurat())
                            <div class="quick-action-item">
                                <a href="{{ route('surat_tugas.approveList') }}">
                                    <span class="quick-action-icon bg-warning text-dark">
                        ...
                        <div class="quick-action-item">
                            <a href="{{ route('surat_tugas.mine') }}">
                                <span class="quick-action-icon bg-info">
                        ...
                        @can('viewAny', App\Models\User::class)
                            <div class="quick-action-item">
                                <a href="{{ route('users.index') }}">
                                    <span class="quick-action-icon bg-success">
                        ...
                        <ul class="list-unstyled mb-0">
                            @forelse ($onlineUsers as $onlineUser)
                                <li class="d-flex align-items-center mb-3">
                                    <div class="online-user-avatar mr-3">
                                        <i class="fas fa-user-circle avatar-icon"></i>
                                    </div>
                                    <div>
                                        <div class="online-user-name">{{ $onlineUser->nama_lengkap }}</div>
                                        <div class="online-user-role text-muted">
                                            {{ $onlineUser->nama_peran ?? 'N/A' }}
                                        </div>
                                    </div>
                                </li>
                            @empty
                                <li class="text-center text-muted small py-3">
                                    Tidak ada pengguna yang online saat ini.
                                </li>
                            @endforelse
                        </ul>
                    </div>
