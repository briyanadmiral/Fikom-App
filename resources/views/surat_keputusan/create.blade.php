...
<style>
    ...
    .page-header .icon{
        ...
        box-shadow:0 1px 10px #6f42c14d;color:#fff;font-size:1.4rem
    }
    .page-header-title{margin:0;font-weight:700;color:var(--purple-700);letter-spacing:-.2px}
    .page-header-desc{margin:.15rem 0 0;color:var(--muted);font-size:.98rem}

    /* ====== Cards ====== */
    .card, .card-settings{border:none;border-radius:var(--card-radius);box-shadow:var(--shadow-sm)}
    ...
    .card-h{
        border-bottom:0;color:#fff;padding:.85rem 1.1rem;border-top-left-radius:var(--card-radius);border-top-right-radius:var(--card-radius)
    }
    ...
    .list-quicknav .list-group-item{
        border:0;border-radius:.6rem;padding:.6rem .8rem;display:flex;align-items:center;gap:.55rem;transition:.15s ease
    }
    ...
    .nomor-builder-preview{font-family:"Courier New",monospace;background:#e9ecef;padding:.25rem .5rem;border-radius:4px}
    ...
    .action-bar{
        position:sticky;bottom:0;left:0;right:0;z-index:998;background:rgba(255,255,255,.94);
    ...
    .tembusan-head{
        ...
        background:linear-gradient(90deg,#5c7cfa 0%,#845ef7 100%);color:#fff
    }
...
@php
    ...
    $tembusanPresets = [
        'Yth. Rektor','Yth. Wakil Rektor I','Yth. Wakil Rektor II','Dekan Fakultas Ilmu Komputer','BAAK','BAUK','BAK',
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-7">
                                ...
                                <div class="d-flex align-items-center mt-2">
                                    ...
                                    <a href="#" class="ml-auto small" id="toggleBuilder">
                                ...
                                <div class="nomor-builder mt-2 p-2" aria-live="polite">
                                    <div class="row gx-2 gy-2">
                                        <div class="col-auto">
                                            <label class="small mb-1" for="no_urut">No Urut</label>
                                            <input type="text" id="no_urut" class="form-control form-control-sm" value="001" style="width:90px">
                                        </div>
                                        <div class="col-auto align-self-end">/</div>
                                        <div class="col-auto">
                                            <label class="small mb-1" for="no_klasifikasi">Klasifikasi</label>
                                            <input type="text" id="no_klasifikasi" class="form-control form-control-sm" value="{{ old('kode_klasifikasi','B.10.1') }}" style="width:140px">
                                        </div>
                                        <div class="col-auto align-self-end">/</div>
                                        <div class="col-auto">
                                            <label class="small mb-1" for="no_unit">Unit</label>
                                            <input type="text" id="no_unit" class="form-control form-control-sm" value="{{ old('unit','TG') }}" style="width:110px">
                                        </div>
                                        <div class="col-auto align-self-end">/UNIKA/</div>
                                        <div class="col-auto">
                                            <label class="small mb-1" for="no_romawi">Bulan</label>
                                            <input type="text" id="no_romawi" class="form-control form-control-sm" value="{{ $currentRomawi }}" style="width:80px">
                                        </div>
                                        <div class="col-auto align-self-end">/</div>
                                        <div class="col-auto">
                                            <label class="small mb-1" for="no_tahun">Tahun</label>
                                            <input type="text" id="no_tahun" class="form-control form-control-sm" value="{{ $currentYear }}" style="width:90px">
                                        </div>
                                    </div>
                                    <small class="text-muted d-block mt-2">
                                        Preview: <span class="nomor-builder-preview" id="nomorPreviewText"></span>
                                    </small>
                                </div>
                            </div>
                            ...
                            <div class="col-md-5">
                                ...
                                <input type="date"
                                       ...
                                       required
                                       value="{{ old('tanggal_asli', now()->format('Y-m-d')) }}">
                            ...
                            <div class="col-12">
                                ...
                                <textarea id="tentang"
                                          ...
                                          placeholder="Contoh: Penetapan Visi, Misi, dan Tujuan Fakultas Ilmu Komputer">{{ old('tentang') }}</textarea>
                            ...
                            <div class="col-12">
                                <div class="tembusan-wrap">
                                    ...
                                    <div class="tembusan-body">
                                        <label for="tembusan-input" class="mb-1">
                ...
                <div class="card card-settings sticky-top" style="top:320px;">
                    ...
                    <div class="card-body">
                        ...
                        <div class="d-grid gap-2">
                            ...
                            <a href="{{ route('surat_keputusan.index') }}" class="btn btn-dark">
...
(function(){
    ...
    function buildNomorString(){
        const urut=String((qs('#no_urut')?.value||'1')).padStart(3,'0');
        const klas=(qs('#no_klasifikasi')?.value||'SK').trim();
        const unit=(qs('#no_unit')?.value||'UNIKA').trim();
        const roma=(qs('#no_romawi')?.value||'I').trim().toUpperCase();
        const thn=(qs('#no_tahun')?.value||new Date().getFullYear()).toString().trim();
        return `${urut}/${klas}/${unit}/UNIKA/${roma}/${thn}`;
    }
    function updateNomorField(){
        const v=buildNomorString();
        if(nomorField) nomorField.value=v;
        const p=qs('#nomorPreviewText'); if(p) p.textContent=v;
    }
    function setNomorMode(isManual){
        if(nomorField) nomorField.readOnly=!isManual;
        builderInputs.forEach(id=>{const el=qs('#'+id); if(el) el.disabled=isManual;});
    ...
    builderInputs.forEach(id=> on(qs('#'+id),'input',()=>{ if(!toggleManual?.checked) updateNomorField(); }));
    on(qs('#toggleBuilder'),'click',e=>{e.preventDefault(); builderBox?.classList.toggle('show');});
    ...
    on(qs('#btn-reserve-nomor'),'click',async function(){
        ...
        try{
            const payload={
                unit:qs('#no_unit')?.value||'',
                kode_klasifikasi:qs('#no_klasifikasi')?.value||'',
                bulan_romawi:qs('#no_romawi')?.value||'',
                tahun:parseInt(qs('#no_tahun')?.value)||new Date().getFullYear()
            };
            ...
            const urutEl=qs('#no_urut'); if(urutEl) urutEl.value=data.no_urut;
            updateNomorField();
    ...
    /* ========== Dynamic Lists (Menimbang, Mengingat, Diktum) ========== */
    ...
    document.addEventListener('click', async function(e){
        ...
        if(e.target.closest('#add-menimbang')){
            ...
            reindexList('menimbang-list','alpha'); acted=true;
        }
            ...
            reindexList('mengingat-list','numeric'); acted=true;
        }
            ...
            initEditor(ta); reindexDiktum(); acted=true;
        }
            ...
            if (d && d.parentElement.children.length>1){
                ...
                d.remove(); reindexDiktum(); acted=true;
            }
        }
    ...
    function updateUI(){
        ...
        const hasErr=id=>!!qs('#section-'+id+' .is-invalid, #section-'+id+' [aria-invalid="true"]');
        ...
        const tanggalEl=qs('[name="tanggal_asli"]');
    ...
    if(sections.length){
        function onScroll(){
            const y=window.scrollY+120; let cur=sections[0];
    ...
    $('#btn-submit-approve').on('click',(e)=>validateAndSubmit(e,'pending'));
    $('#mb-approve').on('click',(e)=>validateAndSubmit(e,'pending'));
    $('#btn-submit-draft').on('click',(e)=>validateAndSubmit(e,'draft'));
    $('#mb-draft').on('click',(e)=>validateAndSubmit(e,'draft'));
    ...
    document.addEventListener('keydown',function(e){
        if(e.ctrlKey||e.metaKey){
            if(e.key==='s' || e.key==='S'){ e.preventDefault(); qs('#btn-submit-draft')?.click(); }
            else if(e.key==='Enter'){ e.preventDefault(); qs('#btn-submit-approve')?.click(); }
        }
    });
})();
</script>
