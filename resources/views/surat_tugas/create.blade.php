                            ...
                            <div class="form-group">
                                ...
                                <div class="row align-items-center">
                                    ...
                                    <div class="col-md-2">
                                        ...
                                        <input type="text" id="nomor_urut" class="form-control text-center" value="{{ old('nomor_urut','') }}" readonly>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="form-group mt-2">
                                ...
                                <div class="input-group">
                                    <input type="text" id="nomor_surat_lengkap_display" class="form-control"
                                        ...
                                        value="(belum disiapkan)" readonly>
                                    <div class="input-group-append">
                                        <button class="btn btn-outline-primary" type="button" id="btn-reserve-nomor" title="Siapkan Nomor">
                                            <i class="fas fa-sync"></i>
                                        </button>
                                        <button class="btn btn-outline-secondary" type="button" id="btn-reset-nomor" title="Reset Nomor">
                                            <i class="fas fa-undo"></i>
                                        </button>
                                    </div>
                                </div>
                                <small class="form-text text-muted">Klik tombol ↻ untuk menyiapkan nomor urut berikutnya dari server.</small>
                                <input type="hidden" id="nomor_surat_lengkap_hidden" name="nomor" value="{{ old('nomor') }}">
                            </div>
...
{{-- ✅ SECURITY FIX: Access denied message --}}
@else
<div class="card card-danger">
    ...
    <div class="card-body">
        <div class="alert alert-danger">
            ...
            Anda tidak memiliki izin untuk membuat Surat Tugas baru.
            Hanya Admin TU yang dapat membuat Surat Tugas.
        </div>
...
<script>
    $(function() {
        ...
        //                  NOMOR SURAT via API
        // =========================================================
        const NOMOR_API = "{{ route('ajax.nomor.reserve') }}";
        const DOC_TYPE = 'ST';
        const UNIT_DISPLAY = 'ST.IKOM'; // ubah bila perlu

        const $disp = $('#nomor_surat_lengkap_display');
        const $hidden = $('#nomor_surat_lengkap_hidden');
        const $urut = $('#nomor_urut').prop('readonly', true);
        ...
        function markNomorStale() {
            if ($manual.val().trim() !== '') return; // kalau manual terisi, jangan tiban
            $disp.val('(belum disiapkan)');
            $hidden.val('');
            $urut.val('');
        }
        async function reserveNomor(showToast = true) {
            // jika manual diisi → pakai manual
            ...
            const tahun = parseInt($('#tahun').val(), 10) || new Date().getFullYear();
            ...
            try {
                ...
                const res = await fetch(NOMOR_API, {
                    method: 'POST',
                    headers: {
        ...
        // reset nomor ketika scope berubah
        $('#klasifikasi_surat, #bulan, #tahun').on('change keyup input', markNomorStale);

        // sinkron saat user mengetik nomor manual
        ...
        // kalau kamu menambahkan tombol #btn-reserve-nomor di UI
        ...
        function populateSpecificTask(selectedKategori, preselectedTugas) {
            ...
            if (found && Array.isArray(found.subtugas) && found.subtugas.length) {
                found.subtugas.forEach(st => {
                    const sel = preselectedTugas === st.nama;
                    $tugasSelect.append(new Option(st.nama, st.nama, sel, sel));
                });
        ...
        // =========================================================
        //       Penerima (internal & eksternal) + status
        // =========================================================
        ...
        function updateStatusPenerima() {
            ...
            for (const id in penerimaState.internal) {
                ...
                if (peranId == 1 || peranId == 6) statusSet.add('Tendik');
                else statusSet.add('Dosen');
            }
            penerimaState.eksternal.forEach(p => {
                const j = (p.jabatan || '').toLowerCase();
                if (j === 'mahasiswa') statusSet.add('Mahasiswa');
                else if (j === 'dosen luar') statusSet.add('Dosen');
                else if (j === 'umum') statusSet.add('Umum');
            });
            ...
            const arr = Array.from(statusSet).sort();
            let displayText = '';
            if (arr.length === 1) displayText = arr[0];
            else if (arr.length === 2) displayText = arr.join(' dan ');
            else displayText = `${arr.slice(0,-1).join(', ')}, dan ${arr[arr.length-1]}`;

            $displayInput.val(displayText);

            // normalisasi ke enum (fix: hilangkan penimpaan nilai)
            const allowed = ['dosen', 'tendik', 'mahasiswa'];
            let normalized = '';
            const lowerTokens = displayText.toLowerCase().split(/[,\s]+dan\s+|,\s*|\s+dan\s+/).map(s => s.trim()).filter(Boolean);
            ...
            if (internalCount === 0 && eksternalCount === 0) {
                ...
                penerimaState.eksternal.forEach((p, i) => {
                    list.append(
                        ...
                        <button type="button" class="btn btn-xs btn-danger remove-penerima" data-type="eksternal" data-id="${i}"><i class="fas fa-times"></i></button>
                    </li>`
                    );
        ...
        function validateForm() {
            ...
            if (!$('#penandatangan').val()) errors.push('Penandatangan wajib dipilih');
            const mulai = $('#waktu_mulai').val(),
                selesai = $('#waktu_selesai').val();
            if (mulai && selesai && new Date(selesai) < new Date(mulai)) errors.push('Waktu selesai harus setelah waktu mulai');
            if (errors.length) {
                Swal.fire({
                    icon: 'error',
