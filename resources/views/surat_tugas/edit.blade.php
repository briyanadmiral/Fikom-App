...
$(function () {
  ...
  window._ = window._ || {};
  if (!_.escape) {
    _.escape = function(s){ return String(s).replace(/[&<>"'`=\/]/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;','/':'&#x2F;','=':'&#x3D;','`':'&#x60;'}[c])); };
  }
  ...
  if (tembusanInput){
    ...
    $(document).on('click', '#btnPasteTembusan', async function(){
      try{
        ...
        const toAdd = items.filter(s => !existing.has(s.toLowerCase())).map(s => ({ value: s }));
        tagify.addTags(toAdd);
      }catch(e){ Swal.fire('Tidak bisa mengakses clipboard','Izinkan akses atau tempel manual.','info'); }
    });
    $(document).on('click', '#btnClearTembusan', function(){ tagify.removeAllTags(); });
  ...
  $('#simpanPenerima').on('click', function() {
    ...
    Array.from(selectedIds).forEach(id => {  // ✅ Gunakan Set
        if (allUsersData[id]) {
